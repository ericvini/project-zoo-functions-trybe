...
function animalsByLocalization() {
  objectanimalsByLocalization = {
  ...
  return objectanimalsByLocalization;
}
function animalsByLocalizationAndName() {
  objectanimalsByLocalizationAndName = {
    NE: animals.filter(animal => animal.location === 'NE').map(animal => ({
      [animal.name]: animal.residents.map(animal => animal.name)
    }

    )
    ),

    NW: animals.filter(animal => animal.location === 'NW').map(animal => ({
      [animal.name]: animal.residents.map(animal => animal.name)
    }
    )
    ),
    SE: animals.filter(animal => animal.location === 'SE').map(animal => ({
      [animal.name]: animal.residents.map(animal => animal.name)
    }
    )
    ),

    SW: animals.filter(animal => animal.location === 'SW').map(animal => ({
      [animal.name]: animal.residents.map(animal => animal.name)
    }
    )
    ),
  ...
  return objectanimalsByLocalizationAndName;
}

function animalsByLocalizationAndNameSorted() {
  objectanimalsByLocalizationAndNameSorted = {
    NE: animals.filter(animal => animal.location === 'NE').map(animal => ({
  ...
  return objectanimalsByLocalizationAndNameSorted;
}
function animalMap(options) {
  let AnimalsByLocalization = {};
  let newOptions = {};
  let listAnimals = '';
  let listAnimalsSort = '';
  Object.assign(newOptions, options);
  if (options === undefined) {
    return animalsByLocalization();


  } if (newOptions['includeNames']) {
    return animalsByLocalizationAndName();
  }

  if (newOptions['sorted']) {
    return animalsByLocalizationAndNameSorted()
  }
}
